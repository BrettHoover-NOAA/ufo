...
#include "ufo/filters/obsfunctions/AircraftBiasCorrectionAscentSquared.h"
...
static ObsFunctionMaker<AircraftBiasCorrectionAscentSquared> makerObsFuncAircraftBiasCorrectionAscentSquared_("AircraftBiasCorrectionAscentSquared");

// -----------------------------------------------------------------------------

AircraftBiasCorrectionAscentSquared::AircraftBiasCorrectionAscentSquared(const eckit::LocalConfiguration & conf)
  : invars_() {
  oops::Log::debug() << "AircraftBiasCorrectionAscentSquared: config = " << conf << std::endl;
  // Initialize options
...
AircraftBiasCorrectionAscentSquared::~AircraftBiasCorrectionAscentSquared() {}

// -----------------------------------------------------------------------------

void AircraftBiasCorrectionAscentSquared::compute(const ObsFilterData & in,
  ...
  in.get(Variable("BiasCoefficientValue/ascentSquaredPredictor"), BCcoeff);
  ...
  // Define a BCprediValue variable
  float BCprediValue;
  for (size_t jj = 0; jj < nlocs; ++jj) {
    if (BCcoeff[jj] != missing && BCpredi[jj] != missing) {
      // ascent predictor = BCprediValue*BCcoeff
      // To match GSI's predictor value, if the ascent value
      // abs(BCpredi) >= 30., the value is set to 30.
      if (abs(BCpredi[jj]) >= 30.) {
        BCprediValue = 30.*30.;
    } else {
        BCprediValue = BCpredi[jj]*BCpredi[jj];
    }
...
const ufo::Variables & AircraftBiasCorrectionAscentSquared::requiredVariables() const {
  return invars_;
}
